    ...
    def postprocess(self, input_image, output):
        ...
        for i in range(rows):
            ...
            if max_score >= self.confidence_thres:
                ...
                new_bbox = self.scale_boxes([640, 640], np.array([outputs[i, :4]]), (self.img_height, self.img_width), xywh=False)

                # Add the class ID, score, and box coordinates to the respective lists
                class_ids.append(class_id)
                scores.append(max_score)
                # boxes.append([left,top,width,height])
                boxes.append([int(new_bbox[0, 0]), int(new_bbox[0, 1]), int(new_bbox[0, 2] - new_bbox[0, 0]),
                              int(new_bbox[0, 3] - new_bbox[0, 1])])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", type=str, default="runs/detect/train5/weights/best.onnx",
                        help="Input your ONNX model.")
    parser.add_argument("--img", type=str, default="VOCdevkit/images/test2007/Image_20191006232359787.jpg",
